...
class TestProfileScopedLockSpare:
    """A profile-scoped HERMES_HOME must not blind the orphan-reap spare scan.

    Regression for the multi-window Desktop SSH takedown (atrium-agents
    2026-08-29): a ``hermes --profile <name> serve --isolated`` backend runs
    with HERMES_HOME pointed at ``<root>/profiles/<name>``. The lock writer
    (windows_ssh_runtime._root()) anchors to the machine root so the locks
    land in ``<root>/desktop-ssh/``; the reap's spare-scan used the
    profile-scoped home instead, found zero locks, and killed every live
    sibling backend owned by other Desktop windows.
    """

    def test_profile_scoped_home_still_spares_lock_owned_pid(
        self, tmp_path, monkeypatch
    ):
        # Real lock layout at the machine root, profile-scoped HERMES_HOME —
        # the exact runtime shape of a ``--profile ruby`` SSH backend.
        root = tmp_path / "root"
        ownership_id = "a" * 32
        spawn_nonce = "b" * 16
        ...
        (lock_dir / "backend.lock.json").write_text(json.dumps({
            ...
            "hermesPath": "/opt/hermes/bin/hermes",
            "hermesHome": str(root),
            "logPath": f"{root}/desktop-ssh/{ownership_id}/{spawn_nonce}.log",
            "startedAt": "2026-08-29T17:08:00Z",
        }))
        # The native home (what get_default_hermes_root anchors to) is the
        # machine root; HERMES_HOME is profile-scoped inside it.
        monkeypatch.setenv("HERMES_HOME", str(root / "profiles" / "ruby"))
        monkeypatch.delenv("HERMES_DESKTOP_CHILD_PID", raising=False)
        monkeypatch.setattr(
        ...
        from hermes_cli.dashboard_procs import _lock_owned_serve_pids

        spared = _lock_owned_serve_pids()

        assert pid in spared, (
            "profile-scoped HERMES_HOME made the reap blind to the machine-root "
            "lock directory; sibling Desktop backends would be killed"
        )

    def test_explicit_base_dir_still_wins(self, tmp_path):
        """The base_dir override (used by tests and direct callers) keeps
        precedence over both the root anchor and HERMES_HOME."""
        ownership_id = "d" * 32
        lock_dir = tmp_path / "elsewhere" / "desktop-ssh" / ownership_id
        lock_dir.mkdir(parents=True)
        (lock_dir / "backend.lock.json").write_text(json.dumps({
            "schemaVersion": 2,
            "protocolVersion": 1,
            "ownershipId": ownership_id,
            "spawnNonce": "e" * 16,
            "tokenFingerprint": "f" * 32,
            "pid": 9876,
            "port": 1,
            "profile": "default",
            "hermesPath": "/opt/hermes/bin/hermes",
            "hermesHome": str(tmp_path / "elsewhere"),
            "logPath": (
                f"{tmp_path}/elsewhere/desktop-ssh/"
                f"{ownership_id}/{'e' * 16}.log"
            ),
            "startedAt": "2026-08-29T17:08:00Z",
        }))
        ...
        assert 9876 in _lock_owned_serve_pids(
            base_dir=tmp_path / "elsewhere" / "desktop-ssh"
        )
