...
def test_release_seam_stops_backend_and_clears_session_state():
    ...
    backend = Mock()
    owner = computer_use._backend_owner_key("session-a")
    computer_use._backends[owner] = backend
    computer_use._backend_call_locks[owner] = computer_use.threading.RLock()
    computer_use._backend_permission_modes[owner] = "unrestricted"
    ...
    assert owner not in computer_use._backend_permission_modes
