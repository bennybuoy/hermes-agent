...
def test_release_seam_stops_exact_backend_and_clears_session_state():
    ...
    first = MagicMock()
    second = MagicMock()
    owner_a = computer_use._backend_owner_key("conversation-a")
    owner_b = computer_use._backend_owner_key("conversation-b")
    computer_use._backends.update({
        owner_a: first,
        owner_b: second,
    })
    computer_use._backend_call_locks.update({
        owner_a: computer_use.threading.RLock(),
        owner_b: computer_use.threading.RLock(),
    })
    ...
    assert owner_a not in computer_use._backends
    assert owner_a not in computer_use._backend_call_locks
    assert "conversation-a" not in computer_use._session_auto_approve
    assert "conversation-a" not in computer_use._always_allow
    assert computer_use._backends[owner_b] is second
...
def test_release_seam_evicts_state_even_when_backend_stop_fails():
    ...
    owner = computer_use._backend_owner_key("failed-run")
    computer_use._backends[owner] = backend
    computer_use._backend_call_locks[owner] = computer_use.threading.RLock()
    computer_use._session_auto_approve["failed-run"] = True

    assert computer_use.release_computer_use_session("failed-run") is True
    assert owner not in computer_use._backends
    assert owner not in computer_use._backend_call_locks
...
def test_release_seam_waits_for_in_flight_action_before_stopping_backend():
    ...
    owner = computer_use._backend_owner_key("cancelled-run")
    computer_use._backends[owner] = backend
    computer_use._backend_call_locks[owner] = call_lock
